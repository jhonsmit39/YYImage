...
CGImageRef YYCGImageCreateWithBPGData(CFDataRef bpgData, BOOL decodeForDisplay)
{
    ...
    if (!bpgData || CFDataGetLength(bpgData) == 0)
        return NULL;
    decoderContext = bpg_decoder_open();
    if (!decoderContext)
        return NULL;
    if (bpg_decoder_decode(decoderContext, CFDataGetBytePtr(bpgData), (int)CFDataGetLength(bpgData)) < 0)
        goto fail;
    if (bpg_decoder_get_info(decoderContext, &imageInfo) < 0)
        goto fail;
    ...
    for (int y = 0; y < height; y++) {
        if (bpg_decoder_get_line(decoderContext, rgbaLine) < 0)
            goto fail;
    ...
    cgImage = CGImageCreate(width, height, 8, 32, stride, YYCGColorSpaceGetDeviceRGB(), bitmapInfo, dataProvider, NULL, NO, kCGRenderingIntentDefault);
