...
// Internal, version-checked, entry point
WEBP_EXTERN(WebPMux *)
WebPMuxCreateInternal(const WebPData *, int, int);
...
//   WEBP_MUX_OK - on success.
WEBP_EXTERN(WebPMuxError)
WebPMuxDeleteFrame(WebPMux *mux, uint32_t nth);
...
//   WEBP_MUX_OK - on success.
WEBP_EXTERN(WebPMuxError)
WebPMuxSetCanvasSize(WebPMux *mux,
                     int width,
                     int height);
...
//   WEBP_MUX_OK - on success.
WEBP_EXTERN(WebPMuxError)
WebPMuxGetCanvasSize(const WebPMux *mux,
                     int *width,
                     int *height);
...
//   WEBP_MUX_OK - on success.
WEBP_EXTERN(WebPMuxError)
WebPMuxGetFeatures(const WebPMux *mux,
                   uint32_t *flags);
...
//   WEBP_MUX_OK - on success.
WEBP_EXTERN(WebPMuxError)
WebPMuxNumChunks(const WebPMux *mux,
                 WebPChunkId id,
                 int *num_elements);
...
//   True on success.
WEBP_EXTERN(int)
WebPAnimEncoderAssemble(WebPAnimEncoder *enc,
                        WebPData *webp_data);
...
WEBP_EXTERN(const char *)
WebPAnimEncoderGetError(WebPAnimEncoder *enc);

// Deletes the WebPAnimEncoder object.
// Parameters:
//   enc - (in/out) object to be deleted
WEBP_EXTERN(void)
WebPAnimEncoderDelete(WebPAnimEncoder *enc);
